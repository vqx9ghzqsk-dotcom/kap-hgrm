...
<head>
    ...
    <style>
        ...
        /* Academic Table styling for Cross-Analysis */
        .academic-table { width: 100%; border-collapse: collapse; margin-top: 15px; background: white; border: 1px solid #e2e8f0; }
        .academic-table th { background: #f8fafc; color: #334155; font-size: 12px; font-weight: 800; text-transform: uppercase; padding: 12px; border-bottom: 2px solid #cbd5e1; text-align: center; }
        .academic-table td { padding: 12px; border-bottom: 1px solid #e2e8f0; font-size: 13px; text-align: center; color: #1e293b; }
        .academic-table tr:hover { background: #f1f5f9; }
        .dash-card { background: white; padding: 25px; border-radius: 20px; border: 1px solid var(--border); box-shadow: 0 4px 6px rgba(0,0,0,0.01); }
        .dash-title { font-weight: 800; color: #0f172a; margin-bottom: 20px; font-size: 14px; text-transform: uppercase; letter-spacing: 0.5px; border-bottom: 2px solid var(--primary-light); padding-bottom: 10px; }
...
<body>
    ...
    <div class="container">
        ...
        <div id="content-3" class="form-content">
            ...
            <div class="section-title">VI. ANALYSES BIVARIÉES (ASSOCIATIONS STATISTIQUES)</div>
            <div id="cross-tables-container" style="display: grid; grid-template-columns: 1fr; gap: 30px;"></div>
        </div>
    ...
    <script type="module">
        ...
        window.updateAnalytics = function () {
            ...
            window.updateCrossTables();
        };

        window.updateCrossTables = function() {
            const container = document.getElementById('cross-tables-container');
            if (!container) return;
            ...
            const getStats = (rows, label, total) => {
                return [label, rows.length, ((rows.length / total) * 100).toFixed(1) + '%'];
            };

            let html = '';

            // Table 1: Niveau d'Étude vs Niveau de Savoir
            const a1 = database.filter(d => d.niveau.includes('A1'));
            const a2 = database.filter(d => !d.niveau.includes('A1'));
            html += createTable("Tableau 1 : Corrélation Niveau d'Étude & Savoir Th.", 
                ["Niveau d'Étude", "Bon Savoir (≥70%)", "Faible/Moy.", "Total"],
                [
                    ["Graduées (A1)", a1.filter(d=>d.scoreSavoir>=70).length, a1.filter(d=>d.scoreSavoir<70).length, a1.length],
                    ["Techniques (A2)", a2.filter(d=>d.scoreSavoir>=70).length, a2.filter(d=>d.scoreSavoir<70).length, a2.length],
                    ["Total", database.filter(d=>d.scoreSavoir>=70).length, database.filter(d=>d.scoreSavoir<70).length, database.length]
                ]
            );

            // Table 2: Service d'Affectation vs Niveau de Savoir
            const services = ['Gynécologie-Obstétrique', 'Médecine Interne', 'Chirurgie', 'Urgences / Autre'];
            const servRows = services.map(s => {
                const sub = database.filter(d => d.service === s);
                return [s, sub.filter(d=>d.scoreSavoir>=70).length, sub.filter(d=>d.scoreSavoir<70).length, sub.length];
            });
            html += createTable("Tableau 2 : Savoir Théorique par Service", 
                ["Service", "Bon Savoir", "Faible/Moy.", "Total"],
                [...servRows, ["Total", database.filter(d=>d.scoreSavoir>=70).length, database.filter(d=>d.scoreSavoir<70).length, database.length]]
            );

            // Table 3: Ancienneté Professionnelle vs Niveau de Savoir
            const exp1 = database.filter(d => d.anciennete < 5);
            const exp2 = database.filter(d => d.anciennete >= 5 && d.anciennete <= 10);
            const exp3 = database.filter(d => d.anciennete > 10);
            html += createTable("Tableau 3 : Expérience vs Savoir Théorique", 
                ["Ancienneté", "Bon Savoir", "Faible/Moy.", "Total"],
                [
                    ["< 5 ans", exp1.filter(d=>d.scoreSavoir>=70).length, exp1.filter(d=>d.scoreSavoir<70).length, exp1.length],
                    ["5 - 10 ans", exp2.filter(d=>d.scoreSavoir>=70).length, exp2.filter(d=>d.scoreSavoir<70).length, exp2.length],
                    ["> 10 ans", exp3.filter(d=>d.scoreSavoir>=70).length, exp3.filter(d=>d.scoreSavoir<70).length, exp3.length],
                    ["Total", database.filter(d=>d.scoreSavoir>=70).length, database.filter(d=>d.scoreSavoir<70).length, database.length]
                ]
            );

            // Table 4: Niveau d'Étude vs Attitude
            html += createTable("Tableau 4 : Niveau d'Étude & Orientation de l'Attitude", 
                ["Niveau", "Attitude Positive", "Attitude Neutre", "Total"],
                [
                    ["A1/LMD", a1.filter(d=>parseFloat(d.scoreAttitude)>3.5).length, a1.filter(d=>parseFloat(d.scoreAttitude)<=3.5).length, a1.length],
                    ["A2/ITM", a2.filter(d=>parseFloat(d.scoreAttitude)>3.5).length, a2.filter(d=>parseFloat(d.scoreAttitude)<=3.5).length, a2.length]
                ]
            );

            // Table 5: Service vs Attitude
            const attRows = services.map(s => {
                const sub = database.filter(d => d.service === s);
                return [s, sub.filter(d=>parseFloat(d.scoreAttitude)>3.5).length, sub.filter(d=>parseFloat(d.scoreAttitude)<=3.5).length, sub.length];
            });
            html += createTable("Tableau 5 : Attitude par Service d'Affectation", 
                ["Service", "Positive", "Neutre/Nég.", "Total"],
                attRows
            );

            // Table 6: Niveau d'Étude vs Qualité de Pratique
            html += createTable("Tableau 6 : Niveau d'Étude & Adéquation des Pratiques", 
                ["Niveau", "Pratique Adéquate", "Pratique Inadéq.", "Total"],
                [
                    ["Graduées (A1)", a1.filter(d=>d.scorePratique>=70).length, a1.filter(d=>d.scorePratique<70).length, a1.length],
                    ["Auxiliaires (A2)", a2.filter(d=>d.scorePratique>=70).length, a2.filter(d=>d.scorePratique<70).length, a2.length]
                ]
            );

            // Table 7: Service vs Pratique
            const pratRows = services.map(s => {
                const sub = database.filter(d => d.service === s);
                return [s, sub.filter(d=>d.scorePratique>=70).length, sub.filter(d=>d.scorePratique<70).length, sub.length];
            });
            html += createTable("Tableau 7 : Qualité de Pratique par Unité de Soin", 
                ["Service", "Adéquate", "Inadéquate", "Total"],
                pratRows
            );

            // Table 8: Corrélation Savoir vs Pratique
            const bonSav = database.filter(d => d.scoreSavoir >= 70);
            const malSav = database.filter(d => d.scoreSavoir < 70);
            html += createTable("Tableau 8 : Lien entre Savoir Théorique et Pratique Clinique", 
                ["Niveau de Savoir", "Pratique Adéq.", "Pratique Inadéq.", "Total"],
                [
                    ["Bon Savoir Th.", bonSav.filter(d=>d.scorePratique>=70).length, bonSav.filter(d=>d.scorePratique<70).length, bonSav.length],
                    ["Savoir Insuffisant", malSav.filter(d=>d.scorePratique>=70).length, malSav.filter(d=>d.scorePratique<70).length, malSav.length]
                ]
            );

            // Table 9: Corrélation Attitude vs Pratique
            const posAtt = database.filter(d => parseFloat(d.scoreAttitude) > 3.5);
            const negAtt = database.filter(d => parseFloat(d.scoreAttitude) <= 3.5);
            html += createTable("Tableau 9 : Lien entre Attitude et Pratique Clinique", 
                ["Type d'Attitude", "Pratique Adéq.", "Pratique Inadéq.", "Total"],
                [
                    ["Attitude Positive", posAtt.filter(d=>d.scorePratique>=70).length, posAtt.filter(d=>d.scorePratique<70).length, posAtt.length],
                    ["Attitude Neutre/Nég.", negAtt.filter(d=>d.scorePratique>=70).length, negAtt.filter(d=>d.scorePratique<70).length, negAtt.length]
                ]
            );

            // Table 10: Âge vs Savoir Théorique
            const ageGroup1 = database.filter(d => d.age_participant < 35);
            const ageGroup2 = database.filter(d => d.age_participant >= 35);
            html += createTable("Tableau 10 : Âge de l'Infirmière & Niveau de Connaissances", 
                ["Tranche d'Âge", "Bon Savoir", "Faible Savoir", "Total"],
                [
                    ["Moins de 35 ans", ageGroup1.filter(d=>d.scoreSavoir>=70).length, ageGroup1.filter(d=>d.scoreSavoir<70).length, ageGroup1.length],
                    ["35 ans et plus", ageGroup2.filter(d=>d.scoreSavoir>=70).length, ageGroup2.filter(d=>d.scoreSavoir<70).length, ageGroup2.length]
                ]
            );
